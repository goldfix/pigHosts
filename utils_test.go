...
func Test_removeComments(t *testing.T) {
	...
	tests := []struct {
		...
		{"remove_comment_1", args{s: "host.local.it #test"}, "host.local.it"},
		{"remove_comment_2", args{s: "host.local.it#test"}, "host.local.it"},
		{"remove_comment_3", args{s: "#host.local.it"}, ""},
		{"remove_comment_4", args{s: "# host.local.it #test"}, ""},
		{"remove_comment_space", args{s: "255.255.255.255                      broadcasthost"}, "255.255.255.255 broadcasthost"},
		{"remove_comment_tab", args{s: "255.255.255.255		broadcasthost"}, "255.255.255.255 broadcasthost"},
		{"remove_comment_tab_upper", args{s: "255.255.255.255		BROADCASTHOST"}, "255.255.255.255 broadcasthost"},
	}
